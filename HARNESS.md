...
**Proposal Strategy는 Proposal Document가 아니다.** 전략은 의사결정 데이터이고 문서는 그것을
렌더링한 결과다. 제안 목표에는 기본값을 두지 않는다 — 조용한 fallback은 파이프라인이 제안의
목적을 정하는 것이다. 제안할 Solution은 호출자가 준 목록에서 **고르는** 것이지 모델이 쓰는
것이 아니며, 제안 문장은 자기가 무엇을 제안하는지 그 목록의 **ref**로 가리켜야 한다.

**산문을 식별자로 쓰지 않는다.** 표시용 text와 참조용 ref를 분리한다 — 자연키로 쓰인 문장은
같은 문구끼리 합쳐지고, 문구를 다듬는 순간 그것을 가리키던 모든 참조가 끊어진다.

**판정하지 않은 것을 기본값으로 채우지 않는다.** 미분류 gap을 가장 이른 시점으로 승격시키면,
기록된 뒤에 판단과 기본값을 구분할 수 없다.
